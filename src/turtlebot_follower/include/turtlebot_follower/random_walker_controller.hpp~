...
private:
  ...
  bool bumper_center_pressed_recede;
  ...
  if (this->getState()) // check, if the controller is active
  {
    ...
    else // kobuki_msgs::BumperEvent::RELEASED
    {
      switch (msg->bumper)
      {
        ...
        case kobuki_msgs::BumperEvent::CENTER:  bumper_center_pressed_ = false;bumper_center_pressed_recede = false; break;
  ...
  if (this->getState()) // check, if the controller is active
  {
	...
	if (turning_){
		if ((ros::Time::now() - turning_start_) < turning_duration_){
			if(flag &&((ros::Time::now() - turning_start_) < turning_duration_*0.5)){
